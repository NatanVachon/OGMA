...
class Character(Box):
    ...
    def predict(self):
        self.prediction, self.predictionImage = ir.predict(self)
    ...
    def to_json(self):
        json_rpz = super().to_json()
        json_rpz["prediction"] = self.prediction
        if self.predictionImage is None:
            json_rpz["image"] = None
        else:
            json_rpz["image"] = self.predictionImage.tolist()
        return json_rpz
...
class Formula(Box):
    ...
    def to_json(self):
        json_rpz = super().to_json()
        json_rpz["characters"] = [char.to_json() for char in self.chars]
        return json_rpz
...
class BlackBoard:
    def __init__(self):
        self.formulas = []
        self.mode = "Declare"  # Blackboard mode corresponds to the mode given to the next created formulas
